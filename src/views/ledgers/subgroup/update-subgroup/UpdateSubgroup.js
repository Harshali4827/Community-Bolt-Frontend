...
const [formData, setFormData] = useState({
  ...
  const fetchSubGroupData = async () => {
    try {
      const response = await axiosInstance.get(`/ledger-subgroup/${id}`);
      console.log('Fetched data:', response.data);
      if (response.data) {
          setFormData(response.data);
        }
    } catch (error) {
      console.error('Error fetching ledger subgroup data:', error);
    }
  };
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axiosInstance.put(`/ledger-subgroup/${id}`,formData);
