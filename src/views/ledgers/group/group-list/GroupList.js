...
const GroupList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosInstance.get(`/ledger-group`);
  ...
  const handleFilter = (event) => {
    ...
    const filteredData = data.filter((row) =>
      String(row.group_name || "").toLowerCase().includes(searchValue) ||
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "ledger group Data");
    XLSX.writeFile(workbook, "LedgerGroup.xlsx");
  };
  const exportPdf = () => {
    const doc = new jsPDF({ orientation: 'landscape' });
    autoTable(doc, {
      head: [["Group name","Status", "Created by"]],
      body: data.map(item => [
        item.group_name,
        item.status,
        item.created_by
...
const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axiosInstance.delete(`/ledger-group/${id}`);
    ...
    <div className="table-container">
      <div className="table-header">
        ...
        <div className="buttons">
          ...
          <button className="btn2"><CSVLink data={data} filename="LedgerGroup.csv" title="CSV" className="csv-link"><FontAwesomeIcon icon={faFileCsv} />
          </CSVLink>
          </button>
        </div>
        <Link to='/ledgers/add-group'>
          <button className="new-user-btn" >+ New Group</button>
        </Link>
      </div>
      ...
      <>
        <FaTimesCircle className="status-icon inactive-icon" />
                  ...
                  <Menu
                    ...
                    onClose={handleClose}>
                     <Link className='Link' to={`/ledgers/update-group/${group.id}`}>
