...
const ChildGroupList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosInstance.get(`/ledger-childgroup`);
...
const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axiosInstance.delete(`/ledger-childgroup/${id}`);
  ...
  return (
    <div className="table-container">
      <div className="table-header">
        ...
        <Link to='/ledgers/add-childgroup'>
        ...
        <tbody>
          {
            currentRecords.length === 0 ? (
              <tr>
                <td colSpan='4'>No child group available</td>
              </tr>
            ) : (
              currentRecords.map((group, index) => (
                <tr key={index}>
                  <td>{index+1}</td>
                  <td>{group.child_group_name}</td>
      ...
      <>
        <FaTimesCircle className="status-icon inactive-icon" />
                  ...
                  <Menu
                    ...
                    onClose={handleClose}>
                     <Link className='Link' to={`/ledgers/update-childgroup/${group.id}`}>
