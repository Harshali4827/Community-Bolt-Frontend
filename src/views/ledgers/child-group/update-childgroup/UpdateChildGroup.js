...
const [formData, setFormData] = useState({
  ...
  const fetchChildGroupData = async () => {
    try {
      const response = await axiosInstance.get(`/ledger-childgroup/${id}`);
      console.log('Fetched data:', response.data);
      if (response.data) {
          setFormData(response.data);
        }
    } catch (error) {
      console.error('Error fetching ledger childgroup data:', error);
    }
  };
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axiosInstance.put(`/ledger-childgroup/${id}`,formData);
...
export default UpdateChildGroup;