    ...
    <div className="table-container">
      <div className="table-header">
        ...
        <div className="buttons">
          ...
          <button className="btn2"><CSVLink data={data} filename="SectorData.csv" title="CSV" className="csv-link"><FontAwesomeIcon icon={faFileCsv} />
          </CSVLink>
          </button>
        </div>
        <Link to='/sectors/add-sector'>
          <button className="new-user-btn" >+ New Sector</button>
        </Link>
      </div>
        ...
        <tbody>
          {
            currentRecords.length === 0 ? (
              <tr>
                <td colSpan='4'>No sectors available</td>
              </tr>
            ) : (
              currentRecords.map((sector, index) => (
                <tr key={index}>
                  ...
                  <td>
                 <span className={`status-text ${sector.status}`}>{sector.status}</span>
                </td>
                   <td>
                    <button
                    className="action-button"
                    onClick={(event) => handleClick(event, sector.id)}>
                    Action
                  </button>
                  <Menu
                    id={`action-menu-${sector.id}`}
                    anchorEl={anchorEl}
